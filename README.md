...
## defines.json
某些Flag的值不只有On和Off两种, 以及某些情况下需要将同一份源码编译为多个.material.bin文件, 这时可以通过在解包输出目录中创建defines.json来添加宏定义规则   
defines.json格式如下:
```json
{
    "material": {
        "Material名(解包输出的json中的name)": ["宏名称"]
    },
    "pass": {
        "Pass名": ["宏名称"]
    },
...
3. 因为RenderChunk.material.bin中的Flag的值都只有On/Off两种, 也无需编译为多个.material.bin文件, 故无需创建`defines.json`以额外添加宏定义规则(其他文件同理视情况创建)   
此时目录结构应为:
```
RenderChunk/
    src/
